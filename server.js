...
const initializeDB = () => {
    db.serialize(() => {
        ...
        db.run(`CREATE TABLE IF NOT EXISTS users (
            ...
            role TEXT NOT NULL CHECK(role IN ('student', 'professor', 'warden'))
        )`);
        ...
        //Create a table for appointments (with fields for user1_id, user2_id, slot_id and status)
...
//     // Insert initial availability 7unj-0lo
//     db.run('INSERT OR IGNORE INTO availability (id, user2_id, start_time, end_time) VALUES (1, 3, "2025-03-21 09:00:00", "2025-03-21 10:00:00")', function (err) {
//         if (err) console.error('Error inserting availability slot 1:', err);
//     });

//     db.run('INSERT OR IGNORE INTO availability (id, user2_id, start_time, end_time) VALUES (2, 3, "2025-03-21 11:00:00", "2025-03-21 12:00:00")', function (err) {
//         if (err) console.error('Error inserting availability slot 2:', err);
//     });

//     db.run('INSERT OR IGNORE INTO availability (id, user2_id, start_time, end_time) VALUES (3, 4, "2025-03-21 13:00:00", "2025-03-21 15:00:00")', function (err) {
//         if (err) console.error('Error inserting availability slot 2:', err);
//     });

//     // Insert initial appointments
//     db.run('INSERT OR IGNORE INTO appointments (id, user1_id, user2_id, slot_id, status) VALUES (1, 1, 3, 1, "booked")', function (err) {
//         if (err) console.error('Error inserting appointment 1:', err);
//     });

//     db.run('INSERT OR IGNORE INTO appointments (id, user1_id, user2_id, slot_id, status) VALUES (2, 2, 3, 2, "booked")', function (err) {
...
app.post('/register', (req, res) => {
    ...
    if (!['student', 'professor', 'warden'].includes(role)) return res.status(400).send('Invalid role.');
...
app.get('/appointments', authenticate, (req, res) => {
    ...
    if (req.user.role === 'professor') return res.status(403).send('Professors cannot view appointments');
    ...
    db.all(`SELECT a.id, a.status, p.email AS professor_email, v.start_time, v.end_time 
            ...
            JOIN users p ON a.user2_id = p.id 
            WHERE a.user1_id = ?`, [req.user.id], (err, appointments) => {
...
app.post('/availability', authenticate, (req, res) => {
    if (req.user.role === 'student') return res.status(403).send('Students cannot add availability');

    const { start_time, end_time } = req.body;

    //Putting the availability into the database
    db.run(`INSERT INTO availability (user2_id, start_time, end_time) VALUES (?, ?, ?)`, [req.user.id, start_time, end_time], function (err) {
...
app.get('/availability/:professorId', authenticate, (req, res) => {
    ...
    db.all(`SELECT * FROM availability WHERE user2_id = ?`, [req.params.professorId], (err, slots) => {
...
app.post('/appointments', authenticate, (req, res) => {
    if (req.user.role === 'professor') return res.status(403).send('Professors cannot book appointments');

    const { user2_id, slot_id } = req.body;

    if (req.user.role === 'student') {
        db.get(`SELECT * FROM users WHERE id = ? AND role = 'professor'`, [user2_id], (err, professor) => {
            if (!professor) return res.status(400).send('Invalid professor ID.');
    
            // Checking if the slot_id exists and belongs to the professor
            db.get(`SELECT * FROM availability WHERE id = ? AND user2_id = ?`, [slot_id, user2_id], (err, slot) => {
                ...
                db.get(`SELECT * FROM appointments WHERE slot_id = ?`, [slot_id], (err, existingAppointment) => {
                    ...
                    db.run(`INSERT INTO appointments (user1_id, user2_id, slot_id, status) VALUES (?, ?, ?, 'booked')`, 
                        [req.user.id, user2_id, slot_id], 
                        function (err) {
        ...
        db.get(`SELECT * FROM users WHERE id = ? AND role = 'warden'`, [user2_id], (err, warden) => {
            if (!warden) return res.status(400).send('Invalid warden ID.');
    
            // Checking if the slot_id exists and belongs to the professor
            db.get(`SELECT * FROM availability WHERE id = ? AND user2_id = ?`, [slot_id, user2_id], (err, slot) => {
                if (!slot) return res.status(400).send('Slot does not exist or does not belong to the warden.');
